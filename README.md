...
- The adjacency matrix $A$ of an undirected graph $G = (V, E)$ is the matrix whose ijth entry $a_{ij}$ is $1$ if $\{i, j\} \in E$ and $0$ otherwise. The degree matrix $D$ of $G$ is a matrix whose $i^{th}$ diagonal entry $d_{ii}$ is the degree $deg(v_i)$ of $v_i \in V$, with the off-diagonal entries being $0$. The Laplacian matrix $L$ of $G$, defined as $D - A$, has the following remarkable properties:
  - The smallest eigenvalue $\lambda_0$ is $0$, with eigenvector $\bold{1}$. The multiplicity of $\lambda_0$ equals the number of connected components of $G$.
  ...
  - [matrix2eigs.py](src/matrix2eigs.py) uses these properties to determine the number of connected components of $G$ and also computes the spectral gap. It also optionally verifies that the off-diagonal entries of $L$ sum to $2*|E| = \sum\limits_{i}deg(v_i)$ as well as the fact that $tr(L) = \sum\limits_{i}\lambda_i = 2*|E|$.
